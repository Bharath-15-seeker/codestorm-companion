...
const AptitudeSheet = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 overflow-hidden">
      ...
      <div className="flex-none bg-white/80 backdrop-blur-md border-b border-slate-100 shadow-sm z-30">
            ...
            <div className="hidden sm:flex flex-col items-end gap-1">
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                    ...
                    pct >= 80 ? "bg-emerald-100 text-emerald-700" : pct >= 40 ? "bg-amber-100 text-amber-700" : "bg-indigo-100 text-indigo-700"
                  )}>
                  {Math.round(pct)}%
                </span>
              </div>
              <div className="w-48 h-2 rounded-full bg-slate-100 overflow-hidden">
                <div
                  className={cn(
                    ...
                    pct >= 80 ? "bg-gradient-to-r from-emerald-400 to-emerald-500" : pct >= 40 ? "bg-gradient-to-r from-amber-400 to-orange-400" : "bg-gradient-to-r from-indigo-400 to-violet-500"
                  )}
                  style={{ width: `${pct}%` }}
                />
              </div>
      ...
      <div className="flex-1 overflow-y-auto scroll-smooth">
        <div className="max-w-5xl w-full mx-auto px-4 sm:px-6 pt-8 pb-20 space-y-4">
          ...
          <Accordion type="multiple" className="space-y-3">
            {topics.map((topic, topicIdx) => {
              ...
              const solvedQ = topic.subTopics.reduce((a, s) => a + s.questions.filter((q) => q.solved).length, 0);
                  ...
                  <AccordionTrigger className="px-5 py-4 hover:no-underline hover:bg-slate-50/60 transition-colors group [&>svg]:hidden">
                    <div className="flex items-center justify-between w-full gap-4">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        ...
                        <svg className="w-4 h-4 text-slate-400 group-data-[state=open]:rotate-180 transition-transform duration-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
                        ...
                        <div key={sub.id}>
                          ...
                          <div className="space-y-3">
                            {sub.questions.map((q, qIdx) => {
                              ...
                              return (
                                <div key={q.id} className={cn("rounded-xl border p-4 space-y-3 transition-all duration-300", q.solved ? "border-emerald-200 bg-emerald-50/40" : q.showResult ? "border-rose-200 bg-rose-50/30" : "border-slate-200 bg-white hover:border-slate-300")}>
                                  <div className="flex items-start justify-between gap-3">
                                    <div className="flex items-start gap-2.5">
                                      <span className={cn("mt-0.5 w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold transition-all duration-300", q.solved ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400")}>
                                        {q.solved ? "✓" : qIdx + 1}
                                      </span>
                                    ...
                                    <span className={cn("flex-shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full", diff.pill)}>
                                      {diff.label}
                                    </span>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {["A", "B", "C", "D"].map((opt) => {
                                      ...
                                      return (
                                        <div
                                          key={opt}
                                          onClick={() => !q.showResult && handleOptionClick(q.id, opt, q.correctOption)}
                                          className={cn("flex items-center gap-2.5 border rounded-lg px-3 py-2.5 text-sm transition-all duration-250 select-none", getOptionStyle(q, opt))}
                                        >
                                          <span className={cn("w-6 h-6 rounded-md flex-shrink-0 flex items-center justify-center text-xs font-bold transition-colors duration-200", isCorrect ? "bg-emerald-500 text-white" : isWrong ? "bg-rose-500 text-white" : "bg-slate-100 text-slate-500")}>
                                            {opt}
                                          </span>
                                  ...
                                  {q.showResult && (
                                    <div className="flex items-center justify-between pt-1 animate-in fade-in slide-in-from-bottom-2 duration-400">
                                      <div className={cn("flex items-center gap-2 text-sm font-semibold px-3 py-1.5 rounded-lg", q.solved ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700")}>
                                        {q.solved ? <><CheckIcon /> Correct! Well done</> : <><XIcon /> Correct answer: <strong>{q.correctOption}</strong></>}
                                      </div>
                                      <button onClick={() => handleRetry(q.id)} className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 px-3 py-1.5 rounded-lg border border-slate-200 hover:border-indigo-200 transition-all duration-200">
                                        <RetryIcon /> Try Again
                                      </button>
                                    </div>
                                  )}
